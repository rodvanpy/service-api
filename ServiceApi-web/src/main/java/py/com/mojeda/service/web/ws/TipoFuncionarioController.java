 ...
 */
@Controller
@RequestMapping(value = "/tipos-funcionarios")
public class TipoFuncionarioController extends BaseController {
    ...
    ResponseListDTO listar(@ModelAttribute("_search") boolean filtrar,
        ...
        TipoFuncionarios model = new TipoFuncionarios();
        model.setActivo("S");
        model.setEmpresa(new Empresas(userDetail.getIdEmpresa()));
        
        List<Map<String, Object>> listMapGrupos = null;
        try {
            inicializarTipoFuncionariosManager();
            Gson gson = new Gson();
            ...
            if (!todos) {
                total = Long.parseLong(tipoFuncionariosManager.list(model).size() + "");
            }
            ...
            listMapGrupos = tipoFuncionariosManager.listAtributos(model, atributos.split(","), todos, inicio, cantidad,
    ...
    ResponseDTO getObject(
        ...
        try {
            ...
            TipoFuncionarios model = tipoFuncionariosManager.get(id);
    ...
    public @ResponseBody
    ResponseDTO create(
            @RequestBody @Valid TipoFuncionarios model,
            Errors errors) {
        ...
        try {
            ...
            TipoFuncionarios dato = new TipoFuncionarios();
            dato.setEmpresa(new Empresas(userDetail.getIdEmpresa()));
            dato.setNombre(model.getNombre());
            
            Map<String,Object> objMaps = tipoFuncionariosManager.getLike(dato,"id".split(","));
            ...
            dato = new TipoFuncionarios();
            dato.setEmpresa(new Empresas(userDetail.getIdEmpresa()));
            
            Integer numeroCodigo = tipoFuncionariosManager.total(dato) + 1;
            
            model.setCodigo("TF-" + numeroCodigo);
            ...
            tipoFuncionariosManager.save(model);
            
            TipoFuncionarios empresa = new TipoFuncionarios();
            empresa.setCodigo(model.getCodigo());
            
            response.setStatus(200);
            response.setMessage("El registro ha sido guardado");           
            response.setModel(tipoFuncionariosManager.get(empresa));
    ...
    ResponseDTO update(
        ...
        try {
            ...
            TipoFuncionarios dato = tipoFuncionariosManager.get(id);
            
            TipoFuncionarios object = new TipoFuncionarios();
            object.setNombre(model.getNombre());
            
            Map<String, Object> objectMaps = tipoFuncionariosManager.getLike(object, "id".split(","));
            if (objectMaps != null
            ...
            tipoFuncionariosManager.update(model);
    ...
    /**
     * Mapping para el metodo DELETE de la vista.(eliminar TipoFuncionarios)
     *
     * @param id de la entidad
     * @return
     */
