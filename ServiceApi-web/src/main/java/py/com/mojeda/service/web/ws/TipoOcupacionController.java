 ...
 */
@Controller
@RequestMapping(value = "/tipos-ocupacion")
public class TipoOcupacionController extends BaseController {
    ...
    ResponseListDTO listar(@ModelAttribute("_search") boolean filtrar,
        ...
        TipoOcupaciones model = new TipoOcupaciones();
        model.setActivo("S");
        ...
        try {
            ...
            if (!todos) {
                total = Long.parseLong(tipoOcupacionesManager.list(model).size() + "");
            }
            ...
            listMapGrupos = tipoOcupacionesManager.listAtributos(model, atributos.split(","), todos, inicio, cantidad,
    ...
    ResponseDTO getObject(
        ...
        try {
            ...
            TipoOcupaciones model = tipoOcupacionesManager.get(id);
    ...
    public @ResponseBody
    ResponseDTO create(
            @RequestBody @Valid TipoOcupaciones model,
            Errors errors) {
        ...
        try {
            ...
            tipoOcupacionesManager.save(model);
            
            
            response.setStatus(200);
            response.setMessage("El Tipo Ocupacion ha sido guardado");           
            response.setModel(tipoOcupacionesManager.get(model));
    ...
    ResponseDTO update(
        ...
        try {
            ...
            tipoOcupacionesManager.update(model);
    ...
    /**
     * Mapping para el metodo DELETE de la vista.(eliminar TipoOcupaciones)
     *
     * @param id de la entidad
     * @return
     */
