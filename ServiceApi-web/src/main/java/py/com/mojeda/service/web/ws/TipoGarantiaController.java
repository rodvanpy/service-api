 ...
 */
@Controller
@RequestMapping(value = "/tipos-garantias")
public class TipoGarantiaController extends BaseController {
    
    String atributos = "id,nombre,codigo,codeudor,hipoteca";
    ...
    ResponseListDTO listar(@ModelAttribute("_search") boolean filtrar,
        ...
        try {
            ...
            if (!todos) {
                total = Long.parseLong(tipoGarantiasManager.list(model).size() + "");
            }
            ...
            listMapGrupos = tipoGarantiasManager.listAtributos(model, atributos.split(","), todos, inicio, cantidad,
    ...
    ResponseDTO getObject(
        ...
        try {
            ...
            TipoGarantias model = tipoGarantiasManager.get(id);
    ...
    public @ResponseBody
    ResponseDTO create(
            @RequestBody @Valid TipoGarantias model,
            Errors errors) {
        ...
        try {
            ...
            tipoGarantiasManager.save(model);
            
            TipoGarantias empresa = new TipoGarantias();
            empresa.setCodigo(model.getCodigo());
            
            response.setStatus(200);
            response.setMessage("El Tipo Garantia ha sido guardado");           
            response.setModel(tipoGarantiasManager.get(empresa));
    ...
    ResponseDTO update(
        ...
        try {
            ...
            tipoGarantiasManager.update(model);
    ...
    /**
     * Mapping para el metodo DELETE de la vista.(eliminar TipoGarantias)
     *
     * @param id de la entidad
     * @return
     */
