...
public class BarriosController extends BaseController {
    ...
    ResponseListDTO listar(@ModelAttribute("fkModel") Long id,
        ...
        Barrios model = new Barrios();
        model.setCiudad(new Ciudades(id));
        ...
        try {
            ...
            if (!todos) {
                total = Long.parseLong(barriosManager.list(model).size() + "");
            }
            ...
            listMapGrupos = barriosManager.listAtributos(model, atributos.split(","), todos, inicio, cantidad,
    ...
    public @ResponseBody
    ResponseDTO create(
            @RequestBody @Valid Barrios model,
            Errors errors) {
        ...
        try {
            ...
            barriosManager.save(model);

            response.setStatus(200);
            response.setMessage("El barrio ha sido guardado");
            response.setModel(barriosManager.get(model));
    ...
    ResponseDTO update(
        ...
        try {
            ...
            Barrios dato = barriosManager.get(id);
