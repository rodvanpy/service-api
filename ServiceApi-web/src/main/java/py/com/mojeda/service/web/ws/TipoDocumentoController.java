...
public class TipoDocumentoController extends BaseController {
    ...
    ResponseListDTO listar(@ModelAttribute("_search") boolean filtrar,
        ...
        try {
            ...
            if (!todos) {
                total = Long.parseLong(tipoDocumentosManager.list(model).size() + "");
            }
            ...
            listMapGrupos = tipoDocumentosManager.listAtributos(model, atributos.split(","), todos, inicio, cantidad,
    ...
    public @ResponseBody
    ResponseDTO create(
            @RequestBody @Valid TipoDocumentos model,
            Errors errors) {
        ...
        try {
            ...
            TipoDocumentos dato = new TipoDocumentos();
            dato.setEmpresa(new Empresas(userDetail.getIdEmpresa()));
            dato.setNombre(model.getNombre());
            
            Map<String,Object> objMaps = tipoDocumentosManager.getLike(dato,"id".split(","));
            ...
            dato = new TipoDocumentos();
            dato.setEmpresa(new Empresas(userDetail.getIdEmpresa()));
            
            Integer numeroCodigo = tipoDocumentosManager.total(dato) + 1;
            ...
            tipoDocumentosManager.save(model);
            
            TipoDocumentos empresa = new TipoDocumentos();
            empresa.setCodigo(model.getCodigo());
            
            response.setStatus(200);
            response.setMessage("El registro ha sido guardado");           
            response.setModel(tipoDocumentosManager.get(empresa));
    ...
    ResponseDTO update(
        ...
        try {
            ...
            TipoDocumentos dato = tipoDocumentosManager.get(id);
            
            TipoDocumentos object = new TipoDocumentos();
            object.setNombre(model.getNombre());
            
            Map<String, Object> objectMaps = tipoDocumentosManager.getLike(object, "id".split(","));
            if (objectMaps != null
