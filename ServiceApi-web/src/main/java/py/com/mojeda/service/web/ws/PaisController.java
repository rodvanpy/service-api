...
public class PaisController extends BaseController {
    ...
    ResponseListDTO listar(@ModelAttribute("_search") boolean filtrar,
        ...
        try {
            ...
            if (!todos) {
                total = Long.parseLong(paisesManager.list(model).size() + "");
            }
            ...
            listMapGrupos = paisesManager.listAtributos(model, atributos.split(","), todos, inicio, cantidad,
    ...
    ResponseDTO getObject(
        ...
        try {
            ...
            Paises model = paisesManager.get(id);
    ...
    public @ResponseBody
    ResponseDTO create(
            @RequestBody @Valid Paises model,
            Errors errors) {
        ...
        try {
            ...
            paisesManager.save(model);

            response.setStatus(200);
            response.setMessage("El Pais ha sido guardado");
            response.setModel(paisesManager.get(model));
    ...
    ResponseDTO update(
        ...
        try {
            ...
            Paises dato = paisesManager.get(id);
