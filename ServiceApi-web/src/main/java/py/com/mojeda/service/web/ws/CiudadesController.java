...
public class CiudadesController extends BaseController {
    ...
    ResponseListDTO listar(@ModelAttribute("id") Long id,
        ...
        try {
            ...
            if (!todos) {
                total = Long.parseLong(ciudadesManager.list(model).size() + "");
            }
            ...
            listMapGrupos = ciudadesManager.listAtributos(model, atributos.split(","), todos, inicio, cantidad,
    ...
    ResponseDTO getObject(
        ...
        try {
            ...
            Ciudades model = ciudadesManager.get(id);
    ...
    public @ResponseBody
    ResponseDTO create(
            @RequestBody @Valid Ciudades model,
            Errors errors) {
        ...
        try {
            ...
            ciudadesManager.save(model);

            response.setStatus(200);
            response.setMessage("La ciudad ha sido guardada");
            response.setModel(ciudadesManager.get(model));
    ...
    ResponseDTO update(
        ...
        try {
            ...
            Ciudades dato = ciudadesManager.get(id);
