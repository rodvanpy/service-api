...
public class TipoIngresoController extends BaseController {
    ...
    ResponseListDTO listar(@ModelAttribute("_search") boolean filtrar,
        ...
        TipoIngresos model = new TipoIngresos();
        List<Map<String, Object>> listMapGrupos = null;
        try {
            inicializarTipoIngresosManager();
            Gson gson = new Gson();
...
//            }
            ...
            if (!todos) {
                total = Long.parseLong(tipoIngresosManager.list(model).size() + "");
            }
            ...
            listMapGrupos = tipoIngresosManager.listAtributos(model, atributos.split(","), todos, inicio, cantidad,
    ...
    ResponseDTO getObject(
        ...
        try {
            ...
            TipoIngresos model = tipoIngresosManager.get(id);
    ...
    public @ResponseBody
    ResponseDTO create(
            @RequestBody @Valid TipoIngresos model,
            Errors errors) {
        ...
        try {
            ...
            TipoIngresos dato = new TipoIngresos();
            dato.setEmpresa(new Empresas(userDetail.getIdEmpresa()));
            
            //Numero Sucursal
            Integer numeroCodigo = tipoIngresosManager.total(dato) + 1;
            ...
            tipoIngresosManager.save(model);
            
            TipoIngresos empresa = new TipoIngresos();
            empresa.setCodigo(model.getCodigo());
            
            response.setStatus(200);
            response.setMessage("El Tipo Ingreso ha sido guardado");           
            response.setModel(tipoIngresosManager.get(empresa));
    ...
    ResponseDTO update(
        ...
        try {
            ...
            TipoIngresos dato = tipoIngresosManager.get(id);
            ...
            tipoIngresosManager.update(model);
